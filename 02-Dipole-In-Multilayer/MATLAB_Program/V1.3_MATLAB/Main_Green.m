...
% This function returns the value of single kx and ky
%% Calculate the RSP
[RSUp, RPUp, RSDn, RPDn, RS21, RP21, RS12, RP12] =Cal_RSP_kxy(num_dl, num_kx, num_ky, Eplist, dl, klz, nUp, nDn);
%%
%pcolor(kx_grid,ky_grid,abs(RSDn(:,:)));shading interp;colormap jet;
...
for l=1:num_kx
    for m=1:num_ky
        [PPUpA(:,:,l,m),PPUpB(:,:,l,m),SSUpA(:,:,l,m),SSUpB(:,:,l,m)]=Cal_Green_1(kx_grid(l,m),ky_grid(l,m),klz(l,m,num_layer));
    end
end
